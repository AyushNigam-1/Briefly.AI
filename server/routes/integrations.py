...
async def add_app_token(payload: AppTokenPayload, current_user: dict = Depends(get_current_user)):
    return await save_app_token(payload, current_user.get("name"))
...
async def get_single_token(app_name: str, current_user: dict = Depends(get_current_user)):
    return await get_specific_app_token(app_name, current_user.get("name"))

@router.get("/{app_id}/login")
async def oauth_login(app_id: str, current_user: dict = Depends(get_current_user)): 
...
async def generic_oauth_callback(app_id: str, code: str, state: str = None):
    ...
    try:
        ...
        raise HTTPException(status_code=401, detail="OAuth flow timed out. Please try again.")
