...
async def get_web_summary(
    ...
    try:
        ...
        await manager.send_message({
            "progress": 0,
            "message": f"Failed to load content from URL. Error: {str(e)}"
        })
        return {"error": str(e)}
    ...
    try:
        ...
        gridfs_file = fs.put(
            img_bytes,
            filename=f"{datetime.utcnow()}_thumbnail.jpg",
        ...
        await manager.send_message({
            "progress": 0,
            "message": f"Thumbnail extraction failed: {str(e)}"
        })
        return {"error": str(e)}

    # ---------- Clean raw content ----------
    await manager.send_message({"progress": 50, "message": "Cleaning content..."})
    cleaned_text = correct_summary(docs[0].page_content, lang)

    # ---------- Prompt ----------
    prompt_template = (
        "Convert the following content into a refined and human-friendly output.\n\n"
        "Action: {format}\n"
        "- shorten → keep only essential points\n"
        "- extend → add explanations and examples\n"
        "- summarize → concise overview\n"
        "- key points → bullet list of actionable items\n\n"
        "Language: {language}\n\n"
        "Content:\n{text}"
    )
    ...
    return save_summary_to_mongo(
        user_id=user_id,
        file_url=file_url,
        corrected_summary=cleaned_text,
        summary=summary,
        title=title,
        type="web",
    )
...
def correct_summary(summary: str, lang: str) -> str:
    correction_prompt = PromptTemplate(
        template=(
            ...
            "{summary}\n\n"
            "Rules:\n"
            "1. Remove ads, copyright notices, unrelated sections\n"
            "2. Keep core content only\n"
