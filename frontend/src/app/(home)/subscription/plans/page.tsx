...
export default function PricingPage() {
    ...
    return (
        <div className="min-h-screen overflow-y-auto bg-[#0b0b0b] text-slate-200 p-6 md:p-12 pb-24 font-mono flex flex-col items-center w-full">
            ...
            {/* Pricing Grid */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-6 max-w-md lg:max-w-6xl w-full">
                {tiers.map((tier) => (
                    <div
                        key={tier.name}
                            ...
                            <ul className="space-y-4">
                                {tier.features.map((feature, idx) => (
                                    <li key={idx} className="flex items-start gap-3 text-sm text-slate-300">
                                        <Check size={18} className={`shrink-0 ${tier.isPopular ? "text-white" : "text-slate-500"}`} />
                                        <span>{feature}</span>
                                    </li>
                                ))}
                            </ul>
