...
const options = [
  ...
  },
  {
    ...
    icon: "/meta.webp",
    isPremium: false,
    description: "Quick search & summarization",
  },
  {
    label: "Qwen 3 32B",
    ...
    icon: "/qwen.webp",
    isPremium: false,
    description: "Strong multilingual & math skills",
  },
  {
    label: "Kimi K2",
    ...
    icon: "/kimi.webp",
    isPremium: false,
    description: "Long context document processing",
  },
  {
    label: "GPT OSS 120B",
    ...
    icon: "/openai.webp",
    isPremium: false,
    description: "Advanced reasoning & complex coding",
  },
  {
    label: "Llama 70B",
    ...
    icon: "/meta.webp",
    isPremium: false,
    description: "Deep analysis & creative writing",
  },
];
...
const ChatInput = ({ query, setQuery, send, isPending, stop }: InputProps) => {
  ...
  return (
    <div className="w-full flex flex-col rounded-2xl border transition-all duration-200 bg-white border-slate-300 dark:bg-tertiary dark:border-secondary shadow-sm relative">
      ...
      <input
        ...
        onKeyDown={(e) =>
          e.key === "Enter" && query && send(query, files, selected.value)
        }
          ...
          <Menu as="div" className="relative flex-shrink-0">
            <MenuButton className="flex items-center gap-2 p-1.5 pr-3 rounded-full transition-colors text-slate-600 hover:bg-slate-100 dark:text-gray-300 dark:hover:bg-white/10 border border-transparent hover:border-slate-200 dark:hover:border-white/10 select-none">
              <img
                ...
                alt={selected.label}
              />
              <span className="text-sm sm:text-base font-medium whitespace-nowrap hidden sm:block">
                {selected.label}
              </span>
              ...
              >
                <button
                  ...
                  onClick={() =>
                    isPending
                      ? stop()
                      : (send(query, files, selected.value), setFiles([]))
                  }
                  className={clsx(
                    "p-2.5 rounded-full flex-shrink-0 transition-all duration-200 flex items-center justify-center",
                    "bg-slate-900 text-white hover:bg-slate-800 dark:bg-primary dark:text-tertiary dark:hover:bg-primary/90 shadow-md scale-100",
