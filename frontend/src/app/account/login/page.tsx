...
const LoginPage: React.FC = () => {
    ...
    // 🌟 The incredibly simple Better Auth social login function
    ...
    return (
        <div className="relative min-h-screen w-full flex items-center justify-center bg-transparent overflow-hidden font-mono text-white px-4 sm:px-0">
            ...
            >
                <div className="bg-[#121212] backdrop-blur-xl border border-white/10 rounded-2xl p-6 sm:p-8 shadow-2xl">
                    <motion.div
                    ...
                    >
                        ...
                        >
                            {loading ? <LoaderIcon className="animate-spin w-5 h-5 sm:w-6 sm:h-6" /> : "Sign In"}
                        </motion.button>

                        <motion.div className="relative flex py-2 items-center" variants={itemVariants}>
                            <div className="flex-grow border-t border-white/10"></div>
                            <span className="flex-shrink mx-3 sm:mx-4 text-gray-500 text-[10px] sm:text-xs">Or continue with</span>
                            <div className="flex-grow border-t border-white/10"></div>
                        </motion.div>
                        ...
                        <motion.div className="flex gap-3" variants={itemVariants}>
                            ...
                            >
                                {socialLoading === 'google' ? <LoaderIcon className="animate-spin w-5 h-5" /> : (
                            ...
                            <button
                                ...
                                className="flex-1 flex items-center justify-center gap-2 bg-white/5 border border-white/10 rounded-xl py-3 sm:py-3.5 hover:bg-white/10 transition-colors disabled:opacity-50"
                            >
                                {socialLoading === 'github' ? <LoaderIcon className="animate-spin w-5 h-5" /> : (
                        ...
                        <motion.div variants={itemVariants} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="mt-2">
                            <Link href='/account/signup' className="w-full flex justify-center bg-transparent border border-white/10 text-white font-medium rounded-xl py-3 sm:py-3.5 hover:bg-white/5 transition-colors text-center text-sm sm:text-base">
                                Create New Account
                            </Link>
                        </motion.div>
                    </motion.form>
